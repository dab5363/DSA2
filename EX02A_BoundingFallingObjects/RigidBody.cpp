...
RigidBody::RigidBody(std::vector<vector3> a_pointList)
{
	...
	m_v3CenterL = vector3(-3.0f);
	m_v3CenterG = vector3(3.0f);

	m_v3HalfWidth = vector3(5.0f, 1.0f, 1.0f);
	m_fRadius = 3.0f;
	m_v3ARBBSize = vector3(1.0f, 5.0f, 1.0f);
}
...
{
	//1) hardcoded to false, you need to calculate BS vs BS
	//2) And for extra credit use ARBB vs ARBB as a pretest, if that 
	//shows true, test the BS vs BS
...
void RigidBody::AddToRenderList(void)
{
	if (m_bVisibleBS)
	{
		if (m_uCollidingCount > 0)
		{
			m_pModelMngr->AddSpecialToRenderList(BTX::eBTX_SPECIALMODEL::SPHERERED,
				glm::translate(m_m4ToWorld, m_v3CenterL) * glm::scale(vector3(m_fRadius)));
		}
		else
		{
			m_pModelMngr->AddSpecialToRenderList(BTX::eBTX_SPECIALMODEL::SPHERECORNFLOWER,
				glm::translate(m_m4ToWorld, m_v3CenterL) * glm::scale(vector3(m_fRadius)));
		}
	}
	if (m_bVisibleOBB)
	{
		if (m_uCollidingCount > 0)
		{
			m_pModelMngr->AddSpecialToRenderList(BTX::eBTX_SPECIALMODEL::CUBEWHITE,
	...
	if (m_bVisibleARBB)
	{
		if (m_uCollidingCount > 0)
		{
			m_pModelMngr->AddSpecialToRenderList(BTX::eBTX_SPECIALMODEL::CUBEYELLOW,
