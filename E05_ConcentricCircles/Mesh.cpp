...
void Mesh::GenerateCCircles(float a_fRadius, int a_nSubdivisions, int a_nCircles, vector3 a_v3Color)
{
	...
	// -------------------------------
	//Find angle for extra circles
	//This is the angle offset each new circle is rotated from the previous one
	float circleRotationAngle = static_cast<float>(2.0f * PI) / static_cast<float>(a_nCircles - 1);

	//Create new points around the circumference using cos(t), sin(t) as many as subdivisions
	for (uint i = 0; i < a_nCircles; i++)
	{
		std::vector<vector3> extraCirclePointsList = pointList;

		matrix4 m4Rotation = glm::rotate(matrix4(), 
			circleRotationAngle * i, 
			vector3(0.0f, 1.0f, 0.0f));

		//Offset 
		for (uint i = 0; i < a_nSubdivisions; i++)
		{
			extraCirclePointsList[i] = m4Rotation * vector4(extraCirclePointsList[i], 1.0f);
		}

		for (uint i = 0; i < a_nSubdivisions; i++)
		{
			AddTri(v3Origin, extraCirclePointsList[i], extraCirclePointsList[(i + 1) % a_nSubdivisions]);
		}
	}
