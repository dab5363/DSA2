...
void Application::Update(void)
{
	...
	static BTX::Model* pPlane = m_pModelMngr->GetModel(sPlane);
	static String sModel = m_pModelMngr->LoadModel("Minecraft\\Creeper.obj");
	static BTX::Model* pModel = m_pModelMngr->GetModel(sModel);
	...
	for (uint i = 0; i < uCount; ++i)
	{
		for (uint j = 0; j < uCount; ++j)
		{
			m_pModelMngr->AddModelToRenderList(pModel,
				...
				BTX::eBTX_RENDER::RENDER_SOLID | BTX::eBTX_RENDER::RENDER_WIRE);
