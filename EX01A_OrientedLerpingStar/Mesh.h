...
class Mesh
{
	...
	BTX::ShaderManager* m_pShaderMngr = nullptr;	//Shader Manager
public:
	/*
...
*/
	...
	*/
	...
	ARGUMENTS:
	-	float a_fRadius -> radius of the star
	-	int a_nRays -> Number of rays coming out of the star
	-	a_v3Color -> Color of the mesh
	OUTPUT: ---
	*/
	void GenerateStar(float a_fRadius, int a_nRays, vector3 a_v3Color = C_WHITE);

	/*
	USAGE: Will Linear interpolate between the stops in the list, given a percentage
	ARGUMENTS:
	-	std::vector<vector3> a_ListOfSteps -> Collection of steps the shape will interpolate in
	-	float a_fPercentage -> Amount of translation between each route (.99 means its 99% done)
	OUTPUT: matrix4 -> current position in world space
	*/
	matrix4 PositionLERP(std::vector<vector3> a_ListOfSteps, float a_fPercentage);
};
