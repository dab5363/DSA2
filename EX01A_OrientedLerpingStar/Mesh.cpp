#include "Mesh.h"
matrix4 Mesh::PositionLERP(std::vector<vector3> a_ListOfSteps, float a_fPercentage)
{
	//Once the final route is done, LERP from last point to first point.
	uint iListSize = a_ListOfSteps.size();
	
	//if less than 2 steps there is nothing to interpolate from
	if (iListSize < 2)
		return matrix4();
	
	//stores the current index of interpolation, so 0 means its interpolating between
	//index 0 and 1 of the list, and 1 means its interpolating between index 1 and 2 etc
	static uint uRoute = 0;
	
	vector3 v3Position; //output position
	// -------------------------------
	...
	*/
	...
	return glm::translate(matrix4(), v3Position);
}

void Mesh::GenerateStar(float a_fRadius, int a_nRays, vector3 a_v3Color)
{
	...
	*/
	...
	AddTri(vector3(0.0f), vector3(1.0f, 1.0f, 0.0f), vector3(0.0f, 1.0f, 0.0f)); //delete this line
