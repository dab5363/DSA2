...
bool RigidBody::IsColliding(RigidBody* const other)
{
	...
	float fDistance = glm::distance(m_v3CenterG, other->m_v3CenterG);
	//distance is less than the sum of their radii?
	float fRadiiSum = this->m_fRadius + other->m_fRadius;

	// Checks if this is outside of the other object bounds on any axis
	//TODO Check if these are checking bounds correctly. Check the zoom for 3/5 about 15min before class is out
...
void RigidBody::SetModelMatrix(matrix4 a_m4ModelMatrix)
{
	...
	//TODO Double check these are correct from around 15 min before class
	...
	RigidBody temp(cornerList);
	m_v3MinG = temp.GetMinLocal();
	m_v3MaxG = temp.GetMinLocal();
}
//Accessors
