...
void Application::ClearScreen(BTX::vector4 a_v4ClearColor)
{
	if (a_v4ClearColor != BTX::vector4(-1.0f))
	{
...
void Application::Init(BTX::String a_sApplicationName, int a_uSize, bool a_bFullscreen, bool a_bBorderless)
{
	...
	//Init System
	m_pSystem = BTX::SystemSingleton::GetInstance();
...
void Application::Init(BTX::String a_sApplicationName, BTX::uint a_uWidth, BTX::uint a_uHeight, bool a_bFullscreen, bool a_bBorderless)
{
	static bool bInitializated = false;
	if (bInitializated)
		return;
	...
	m_v4ClearColor = BTX::vector4(BTX::C_BLUE_CORNFLOWER, 1.0f);
	...
	//Init Camera
	m_pCameraMngr = BTX::CameraManager::GetInstance();
	m_pCameraMngr->SetPositionTargetAndUpward(
		BTX::vector3(0.0f, 0.0f, 15.0f),//Camera position
		BTX::vector3(0.0f, 0.0f, 0.0f),//What I'm looking at
		BTX::AXIS_Y);//What is up
	
	//Init Model and Entity Managers
	m_pModelMngr = BTX::ModelManager::GetInstance();
	m_pEntityMngr = BTX::EntityManager::GetInstance();
...
void Application::InitWindow(BTX::String a_sWindowName)
{
	BTX::uint uStyle = sf::Style::Default;
...
{
	BTX::String sRoot = m_pSystem->m_pFolder->GetFolderRoot();
	BTX::String App = m_pSystem->GetAppName();
	App = sRoot + App + ".ini";

	BTX::FileReader reader;
	//If the file doesn't exist, exit this method
	if (reader.ReadFile(App.c_str()) == BTX::OUT_ERR_FILE_MISSING)
	{
		return;
	}
	...
	reader.Rewind();
	while (reader.ReadNextLine() == BTX::OUT_RUNNING)
	{
		BTX::String sWord = reader.GetFirstWord();
		...
		else if (sWord == "FrameRate:")
		{
			...
			m_pSystem->SetMaxFrameRate(static_cast<BTX::uint>(nValue));
		}
		...
		else if (sWord == "Background:")
		{
			...
			m_v4ClearColor = BTX::vector4(fValueX, fValueY, fValueZ, fValueW);
		}
		else if (sWord == "Data:")
		{
...
matrix4 Application::OrientByArcball(void) { return BTX::ToMatrix4(m_qArcBall); }