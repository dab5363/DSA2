...
void Application::InitVariables(void)
{
	...
	for (uint i = 0; i < uCount; i++)
	{
		Entity* tempEnt = m_pMyEntityMngr->GetEntity(i);
		RigidBody* tempRigidBody = tempEnt->GetRigidBody();
		pointList.push_back(tempRigidBody->GetMinGlobal());
		pointList.push_back(tempRigidBody->GetMaxGlobal());
	}

	m_pRigidBody = new RigidBody(pointList);
