...
void Mesh::GenerateCone(float a_fRadius, float a_fHeight, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	//Matches the solution and the final screenshot in the instructions
	quaternion rotateQuaternion = glm::angleAxis(glm::radians(45.0f), vector3(1.0f, 1.0f, 0.0f));
	//Matches the written instruction to rotate on all three axis
	//quaternion rotateQuaternion = glm::angleAxis(glm::radians(45.0f), vector3(1.0f, 1.0f, 1.0f));

	for (uint i = 0; i < m_uVertexCount; i++)
	{
		m_lVertexPos[i] = glm::rotate(rotateQuaternion, m_lVertexPos[i]);
	}
