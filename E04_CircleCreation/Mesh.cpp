...
void Mesh::GenerateCircle(float a_fRadius, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	// Replace this with your code
	//Find angle for each subdivision which will be used with sin and cos to find the end points
	float subAngle = (2.0f * PI) / a_nSubdivisions;
	std::vector<vector3> circlePoints;

	//Make the outer ring of points for the circle
	for (uint i = 0; i < a_nSubdivisions; i++)
	{
		vector3 perimeterPoint(glm::cos(subAngle * i) * a_fRadius,
			glm::sin(subAngle * i) * a_fRadius,
			0.0f);

		//Add point to list of point for the circle
		circlePoints.push_back(perimeterPoint);
	}

	//Origin for circle and point that all triangles share
	vector3 circleOrigin(0,0,0); 

	for (uint i = 0; i < a_nSubdivisions; i++)
	{
		AddTri(circleOrigin, circlePoints[i], circlePoints[(i + 1) % a_nSubdivisions]);
	}
	// -------------------------------
