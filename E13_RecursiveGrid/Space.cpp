...
Space::Space(uint a_nSubdivisionsX, uint a_nSubdivisionsY, uint a_nSubdivisionsZ)
{
	...
	*/
	...
	m_uNodeCount = 0;
	m_uID = m_uNodeCount;
	m_uSubX = pow(m_uSubX, a_nSubdivisionsX);//Number of divisions in X
	m_uSubY = pow(m_uSubY, a_nSubdivisionsY);//Number of divisions in Y
	m_uSubZ = pow(m_uSubZ, a_nSubdivisionsZ);//Number of divisions in Z
...
void Space::Subdivide(void)
{
	...
	*/
	...
	//{
	//	// When dividing these by two this results in 3 then 8 then 18 then 22 subdivisions and does not properly divide the area
	//	// When subtracting by one this results in 6 then 24 and does not properly divide the area
	//	m_pChild[i]->m_uSubX = m_uSubX - 1;
	//	m_pChild[i]->m_uSubY = m_uSubY - 1;
	//	m_pChild[i]->m_uSubZ = m_uSubZ - 1;
